"""Endpoint for gift cards transactions
"""
...
class TranscationsInternal(Resource):
    @api.marshal_list_with(default_return)
    @token_required()
    def get(self, user_data):
        """GET method for gift card transactions from internal database
        ...
        for item in transactions_data:
            item["key"] = key # Unique key for front-side table rendering
            item["transaction_date"] = item["transaction_date"].strftime("%d/%m/%Y") # Datetime to string since it is not JSON serializable
...
@api.route("/external")
@api.doc(responses = responses, security = "jwt")
class TranscationsExternal(Resource):
    @api.marshal_list_with(default_return)
    @token_required()
    def get(self, user_data):
        """GET method for gift card transactions from internal database
        ...
        for item in transactions_data:
            item["key"] = key # Unique key for front-side table rendering
            item["transaction_date"] = datetime.strptime(item["transaction_date"], "%Y-%m-%d").strftime("%d/%m/%Y") # Datetime to string since it is not JSON serializable
            key += 1
